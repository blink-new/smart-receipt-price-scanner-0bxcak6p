...
export async function reverseGeocode(lat: number, lng: number): Promise<LocationResult> {
  try {
    ...
    // First try to determine the general area based on coordinates
    const estimatedLocation = getLocationEstimate(lat, lng)
    
    // Use web search to find more specific location information
    const searchResults = await blink.data.search(`${lat},${lng} address location ${estimatedLocation.city} ${estimatedLocation.province} Canada`, {
      type: 'web',
      limit: 5
    })
    ...
    const { object: locationData } = await blink.ai.generateObject({
      prompt: `Analyze these search results for coordinates ${lat}, ${lng} and extract the complete address information. The coordinates are estimated to be near ${estimatedLocation.city}, ${estimatedLocation.province}.

Search results:
${searchResults.organic_results?.map(result => `${result.title}: ${result.snippet}`).join('\n') || 'No results found'}

Based on the coordinates ${lat}, ${lng}, provide the most accurate Canadian address information possible. If specific street address isn't found, use the city/neighborhood level information. Ensure the city and province are realistic for these coordinates.`,
      schema: {
        type: 'object',
        properties: {
          address: { type: 'string' },
    ...
    return {
      ...
      city: locationData.city || estimatedLocation.city,
      province: locationData.province || estimatedLocation.province,
    ...
    // Fallback to estimated location based on coordinates
    const estimatedLocation = getLocationEstimate(lat, lng)
